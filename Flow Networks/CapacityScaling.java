...
public class CapacityScaling {
    ...
    public CapacityScaling(int numVertices) {
    ...
    public int[] minCostMaxFlow(int s, int t) {
        ...
        for (int u = 0; u < numVertices; u++) {
            for (int v = 0; v < numVertices; v++) {
                delta = Math.max(delta, capacity[u][v]);
            }
        }
    ...
    private boolean spfa(int s, int t) {
        ...
        while (head != tail) {
            ...
            for (int v = 0; v < numVertices; v++) {
                if (capacity[u][v] - flow[u][v] >= delta && dist[v] > dist[u] + cost[u][v]) {
    ...
    public static void main(String[] args) {
        ...
        int[] result = cs.minCostMaxFlow(0, 3);
